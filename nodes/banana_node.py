...
class BananaImageGenerationNode(comfy_io.ComfyNode):
    ...
    def execute(cls, api_provider, host_type, prompt, model, aspect_ratio, image_size, seed, timeout, max_retries, poll_interval, save_to_output, 
        ...
        try:
            ...
            # ===== 外层重试机制：实时读取 config 中 banana_nodes.max_retry =====
            # 无需重启 ComfyUI，直接修改 config.json 即可生效（_load_config 每次执行都重新读取）
            retry_limit = int(config.get("banana_nodes", {}).get("max_retry", 0) or 0)
            for attempt in range(retry_limit + 1):
                if attempt > 0:
                    log(f"开始第 {attempt} 次重试（上限 {retry_limit} 次）", "🔄")
                try:
                    ...
                    if attempt >= retry_limit:
                        log(f"已达最大重试次数（{retry_limit}），抛出最终错误", "⏰")
                        raise
                    log(f"本次执行失败，开始重试（第 {attempt + 1}/{retry_limit} 次）: {str(e)}", "🔄")
        except Exception as e:
