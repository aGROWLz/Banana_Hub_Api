...
class SeedreamImageGenerationNode(comfy_io.ComfyNode):
    ...
    支持 KK 中转站 doubao-seedream-4.0/4.5/3.0 图像生成
    """
        ...
        return comfy_io.Schema(
            ...
            inputs=[
                ...
                comfy_io.Combo.Input(
                    "model",
                    options=models,
                    default=models[0] if models else "doubao-seedream-4.5"
                ),
                comfy_io.Combo.Input(
                    "image_size",
                    options=image_sizes,
    ...
    @classmethod
    def execute(cls, prompt, model, image_size, seed, n, timeout) -> comfy_io.NodeOutput:
        ...
        try:
            ...
            if not provider:
                error_msg = "未找到 Seedream API 提供商配置 (KK_seedream_api)"
                log(error_msg, "❌")
            ...
            if not api_key:
                error_msg = "错误: 未设置 API Key，请在配置文件的 api_keys.KK_seedream_api 中设置"
                log(error_msg, "❌")
                raise ValueError(error_msg)
