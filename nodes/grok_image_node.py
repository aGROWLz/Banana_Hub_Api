...
from ..utils import APILoader, build_request_payload


class _GrokImageBaseNode(comfy_io.ComfyNode):
    api_loader = None
    provider_id = ""
    ...
    def _execute_request(cls, *, request_name, payload, timeout, save_to_output, input_images, log, host_type):
        ...
        api_key = config.get("api_keys", {}).get("grok_image_api", "")
        if not api_key:
            raise ValueError("错误: 未设置 API Key，请在配置文件的 api_keys.grok_image_api 中设置")
...
class GrokImageNode(_GrokImageBaseNode):
    provider_id = "grok_image_api"
    log_prefix = "Grok-Image"
    save_prefix = "grok_image"
    ...
    def define_schema(cls) -> comfy_io.Schema:
        provider = cls._get_provider()
        models = provider.models if provider and provider.models else [
